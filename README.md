...
- Поддерживает цветовые текстуры `map_Kd` в PPM/PNG/JPG.
- Поддерживает карты нормалей, шероховатости и металличности для OBJ/MTL.
- Поддерживает базовые карты нормалей для OBJ.
- Поддерживает простой импорт glTF/GLB для демонстрационных сцен.
...
Например, glTF поддерживается только в простом варианте: `.gltf` + внешний `.bin` или `.glb` с JSON/BIN-частями, геометрия, базовые материалы, базовые текстуры и преобразования узлов. Анимации, скелетная анимация, morph targets и сложные расширения glTF не поддерживаются.
...
качество, шумоподавитель и FPS/GPU time. Справа есть ImGui-панель с вкладками:
`Сцена`, `Объекты`, `Материалы`, `Свет`, `Качество`, `Диагностика`. В ней можно
мышкой выбрать сцену, загрузить OBJ/glTF/GLB, добавить или удалить сферу,
изменить положение сферы, положение/поворот/масштаб модели, явно выбрать тип
материала, настроить свет, экспозицию, качество и шумоподавитель. Панель можно
изменять по размеру и прижимать к правому краю окна.
...
При запуске приложение пишет понятные предупреждения в консоль и в файл
`RayTracerRTX.log` в текущей рабочей папке. Этот файл удобно открыть, если
модель, текстура или JSON-сцена не загрузились.

Если отсутствует или повреждена текстура, карта нормалей, карта шероховатости,
карта металличности, MTL-файл или карта окружения, программа не должна падать:
она использует цвет и параметры материала из файла сцены или безопасные значения
