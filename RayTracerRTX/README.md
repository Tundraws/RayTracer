...
модели, сферы, типа материала модели или сферы, качества, добавления и
удаления сфер, изменения размера и цвета сферы, а также настройки экспозиции,
неба, света, шероховатости, прозрачности и показателя преломления. В этой же
панели есть кнопка `Загрузить модель...`: она открывает стандартное окно выбора
файла Windows и позволяет добавить в демонстрацию `.obj` или `.gltf` модель без
